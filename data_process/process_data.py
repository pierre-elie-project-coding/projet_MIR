...
def fetch_data_for_training():
    ...
    df = read_data_from_text(path_read_data=path_read_data,path_read_par=path_read_par,path_read_sol=path_read_sol,stop=16)
    ...
    df_data_padded = df_data.apply(lambda x: pad_sequence(x, max_len)) # padding so all rows are the same length
    ...
    # TODO : find a different solution as putting 0 ruins the data
    df_sol_padded = df_sol.apply(lambda x: pad_sequence(x, max_len)) # padding so all rows are the same length
